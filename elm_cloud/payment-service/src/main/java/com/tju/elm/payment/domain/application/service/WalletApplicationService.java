...
import com.tju.elm.payment.domain.model.enums.TransactionType;
...
public class WalletApplicationService {
    ...
    public Boolean payOrder(Long orderId) {
        ...
        // 设置订单已支付状态
        orderClient.setOrderStatus(1,orderId,null);
        // 设置支付方式
        orderClient.setPaymentStatus(orderId,2);

        // 订单支付完成，发送消息到RabbitMQ（异步通知营销系统计算积分）
        // 注意：积分兑换订单不需要发送消息，因为积分已扣除，不需要再发放奖励积分
        if (!isPointsExchangeOrder(orderId)) {
            orderClient.sendOrderPaidMessage(orderId);
        }
...
//        wallet.pay(amount.add(fee));
    ...
    public Boolean applyVip(Integer toVipLevel) {
        ...
        try {
            ...
            if (memberLevel != null && memberLevel >= 1 && memberLevel <= 3) {
                pointClient.updateVip(user.getId(), memberLevel);
            }
        } catch (Exception e) {
            // 积分处理失败不影响会员升级，记录日志
    ...
    public Boolean repayLoan(Long id,Integer option) {
        ...
        // 还贷款行为积分奖励
        try {
            pointClient.countPointsByType("repay_loan");
    ...
    public boolean thawTransactionAccount(Long  orderId,Integer status) {
        Transaction transaction = transactionRepository.getTransactionByOrder(orderId);
        transactionRepository.thawTransaction(transaction.getId(), status);
        return true;
    }

    public boolean income(Long userId,Long orderId) {
        Transaction transaction = transactionRepository.getTransactionByOrder(orderId);
        Wallet wallet = walletRepository.findByUserId(userId);
    ...
    public boolean back(TransactionDTO backTransaction) {
        Wallet my_wallet = walletRepository.findByUserId(backTransaction.getFromUserId());
        Wallet bu_wallet = walletRepository.findByUserId(backTransaction.getToUserId());


        Transaction transaction = transactionRepository.getTransactionByOrder(backTransaction.getOrderId());
        ...
        transactionRepository.payOrder(back_transaction, backTransaction.getOrderId());

        my_wallet.collection(transaction.getAmount());
