  ...
  export default {
	...
	setup() {
	  ...
	  onMounted(() => {
		...
		axios
		  ...
		  .then((response) => {
			...
			foodArr.value = response.data.map((item) => ({
			  ...item,
			  quantity: 0,
			}));
			if (user.value !== null) {
		...
		axios
		  ...
		  .then((response) => {
			...
			foodArr.value.forEach((foodItem) => {
			  foodItem.quantity =
				cartArr.find((cartItem) => cartItem.foodId === foodItem.foodId)
				  ?.quantity || 0;
			});
		  })
		  .catch((error) => {
	  ...
	  const totalSettle = computed(
		() => totalPrice.value + business.value.deliveryPrice
	  );
